...
def calculate_daily_calories(ideal_weight, species):
    """
    Calculates recommended daily calorie range for the pet's ('dog' or 'cat') ideal weight, which is
    the RER of ideal weight. Returns a rounded integer.
    """

    # Convert ideal weight from lbs to kg for calculations
    ideal_weight_kg = ideal_weight / 2.20462

    rer_pow = RER_POW[species]

    # Calculate RER (Resting Energy Requirement) for ideal weight
    ideal_rer_kcal = 70 * ideal_weight_kg ** rer_pow

    return int(round(ideal_rer_kcal, -1))
...
Weight: 102.8
BCS: 7
Recommended kcal/day for goal weight: 1090
"""